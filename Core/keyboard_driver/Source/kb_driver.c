...
const uint16_t row_pins[KEY_ROWS] = { R0_Pin, R1_Pin, R2_Pin, R3_Pin, R4_Pin,
R5_Pin };

GPIO_TypeDef *const row_ports[KEY_ROWS] = { R0_GPIO_Port, R1_GPIO_Port,
R2_GPIO_Port,
...
void resetRNBD350(bool state) {
	if (state) {
		...
		if ((RNBD.gpio.reset_port->ODR & RNBD.gpio.reset_pin)
				!= RNBD.gpio.reset_pin) {
...
void RxIndicate(bool value) {
	if (value) {
		...
		if ((RNBD.gpio.reset_port->ODR & RNBD.gpio.wakeup_pin)
				!= RNBD.gpio.wakeup_pin) {
			LOG_ERROR("ERR: Reset pin failed to release");
		}
	}
}
...
void setBLECallbacks(void) {
	RNBD.callback.delayMs = non_blocking_delay_ms;
	...
	while (retry <= retries) {
		if (RNBD_Init()) {
			...
			if (RNBD_EnterCmdMode()) {
				...
				if (!GATT_ble_Init()) {
					LOG_ERROR(
							"Bluetooth GATT HID profile configuration failed");
					...
					LOG_INFO(
							"Re-attempting Initialisation After Factory Reset");
				...
				LOG_DEBUG("BLE: Output - %d | Connected - %d",
						kb_state.output_mode, RNBD.connected);
...
void sendUSBReport(void) {
	...
	if (memcmp(&kb_state.current_report, &last_report_usb, sizeof(hid_report_t))
			!= 0) {
		__disable_irq();
		uint8_t result = USBD_HID_SendReport(&hUsbDeviceFS,
				(uint8_t*) &kb_state.current_report, sizeof(hid_report_t));
		__enable_irq();

		if (result == USBD_OK) {
			kb_state.last_report.usb_last_report = current_time;
			memcpy(&last_report_usb, &kb_state.current_report,
					sizeof(hid_report_t));
			clearReport();
		}
	}
...
void sendBLEReport(void) {
	...
	if (memcmp(&kb_state.current_report, &last_report, sizeof(hid_report_t))
			!= 0) {

		if (GATT_sendHIDReport((uint8_t*) &kb_state.current_report,
				sizeof(ble_hid_report_t))) {
			// Update timing and save last report
			kb_state.last_report.ble_last_report = current_time;
			memcpy(&last_report, &kb_state.current_report,
					sizeof(hid_report_t));
			clearReport();
